...
class BuildTester:
    ...
    def test_dependencies(self) -> bool:
        ...
        if self.platform == "linux":
            try:
                ...
                result = subprocess.run(
                    ["ldd", str(self.executable_path)], check=False, capture_output=True, text=True
                )
    ...
    def test_embedded_resources(self) -> bool:
        ...
        if 20 <= size_mb <= 200:
            ...
            print(
                f"   Executable size ({size_mb:.1f} MB) seems too small - resources might be missing"
            )
            return False
        else:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Test built iPhoto Downloader executable")
    parser.add_argument("executable", help="Path to the executable to test")
    parser.add_argument("--summary-only", action="store_true", help="Show only test summary")
