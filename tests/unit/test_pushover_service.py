...
from src.icloud_photo_sync.pushover_service import (
    PushoverConfig,
...
class TestPushoverNotificationService:
    ...
    @patch('src.icloud_photo_sync.pushover_service.requests.post')
    ...
    def test_service_without_device(self):
        ...
        with patch('src.icloud_photo_sync.pushover_service.requests.post') as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {"status": 1}
            mock_post.return_value = mock_response
