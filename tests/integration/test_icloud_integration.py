...
class TestiCloudIntegration:
    ...
    def test_config_loads_with_keyring_credentials(self):
        ...
        assert config.icloud_username is not None, "Username should be available from keyring or env"
        assert config.icloud_password is not None, "Password should be available from keyring or env"
    ...
    def test_icloud_authentication_without_2fa(self):
        ...
        if not auth_result:
            pytest.skip("Authentication failed - this may be due to 2FA requirement or invalid credentials")
...
class TestiCloudInteractive:
    ...
    def test_interactive_2fa_authentication(self):
        ...
        config = get_config()
        setup_logging(config)  # Set up logging for interactive tests
        client = iCloudClient(config)
