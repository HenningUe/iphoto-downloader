...
class InstanceManager:
    ...
    def _acquire_lock_windows(self) -> bool:
        ...
        try:
            self.lock_file_handle = os.open(
                str(self.lock_file_path), os.O_CREAT | os.O_WRONLY | os.O_TRUNC
            )
    ...
    def get_running_instance_info(self) -> str | None:
...
def validate_multi_instance_config(allow_multi_instance: bool) -> bool:
    ...
    if not isinstance(allow_multi_instance, bool):
        raise ValueError(
            f"allow_multi_instance must be a boolean, got {type(allow_multi_instance)}"
        )
