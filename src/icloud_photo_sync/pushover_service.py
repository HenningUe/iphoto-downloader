...
class PushoverNotificationService:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response_data.get("status") == 1:
                    ...
                    logger.error(f"Pushover API error: {response_data.get('errors', 'Unknown error')}")
                    return False
            else:
                logger.error(f"Pushover API request failed with status {response.status_code}: {response.text}")
                return False
    ...
    def send_auth_success_notification(self, username: str) -> bool:
        ...
        try:
            ...
            message = f"2FA authentication completed successfully for '{username}'. Photo sync will continue."
